...
bool Battle::ShowSkills() {
	while (1) {
		...
		if (damage) {
			monsterbeaten();
			Battlelog("%y%s%wÀº/´Â %r%d%wÀÇ µ¥¹ÌÁö¸¦ %y%s%w¿¡°Ô ÁÖ¾ú´Ù!", p.name, damage, monster.name);
		}
		else {
			Battlelog("%y%s%wÀº/´Â ¸öÀ» Ãß½º·È´Ù!", p.name);
		}
		return true;
	}
...
void Battle::AIBattle(){
	while (1) {
		if (RandInt(2)) {
			...
			Battlelog("%y%s%wÀº/´Â %y%s%wÀ»/¸¦ °ø°ÝÇß´Ù!",monster.name,p.name);
			Battlelog("%y%s%wÀº/´Â %r%d%wÀÇ µ¥¹ÌÁö¸¦ ÀÔ¾ú´Ù!",p.name, monsterdmg);
			break;
		}
		else {
			//¸ó½ºÅÍ½ºÅ³
			for (int i = 0; i < SKILL_NUM; i++) {
				if (monster.sk_list[i]) {//½ºÅ³ ÀÖÀ»¶§
					if (monster.getAP() >1 ) {//½ºÅ³»ç¿ëap°¡ ÇöÀçAPº¸´Ù ³ôÀ¸¸é
						...
						Battlelog("%y%s%wÀº/´Â%y%s%wÀ»/¸¦ »ç¿ëÇß´Ù!", monster.name, sk_name[i]);
						if (damage > 0) {
							playerbeaten();
							...
							Battlelog("%y%s%wÀº/´Â ¸öÀ» Ãß½º·È´Ù!", monster.name);
						}
					}
				}
			}
